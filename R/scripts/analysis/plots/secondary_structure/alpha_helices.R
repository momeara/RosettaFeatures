...
plot_id <- "hbond_backbone_backbone_alpha_type_AHdist_by_sample_source"
p <- ggplot(data=dens) + theme_bw() +
...
p <- ggplot(data=qs) + theme_bw() +
	geom_line(aes(y=probs, x=quantiles, colour=alpha_type)) +
	facet_wrap(~sample_source) +
	ggtitle(paste("Alpha Helix H-Bond AHD CDF, B-Fact < 30", sep="")) +
	...
	geom_indicator(aes(colour=alpha_type, indicator=counts, group=alpha_type), ypos=.9, yjust="bottom") +
...
plot_id = "alpha_type_hbond_AHD_CDF_by_sample_source"
p <- ggplot(data=qs) + theme_bw() +
	geom_line(aes(y=probs, x=quantiles, colour=sample_source)) +
	facet_wrap(~alpha_type, nrow=2) +
	ggtitle(paste("Alpha Helix H-Bond AHD CDF, B-Fact < 30", sep="")) +
	scale_y_continuous("Fraction", limit=c(0,1), breaks=c(0,.25, .50, .75)) +
	scale_x_continuous("Angle Deviation from Linear for Acceptor -- Hydrogen -- Donor (degrees)") +
...
plot_id <- "hbond_backbone_backbone_alpha_type_cosBAH_sample_source"
p <- ggplot(data=dens) + theme_bw() +
...
plot_id <- "hbond_backbone_backbone_alpha_type_chi_sample_source"
p <- ggplot(data=dens) + theme_bw() +
	geom_line(aes(x=x, y=y, colour=sample_source)) +
	geom_indicator(aes(indicator=counts, colour=sample_source, group=sample_source)) +
	facet_wrap( ~ alpha_type) +
