...
dens_est <- estimate_density_1d(
	f, c("sample_source", "acc_chem_type", "don_chem_type"),
	"AHdist", weight_fun = radial_3d_normalization)


dens_est$y_energy <- -log(dens_est$y)*.35

dens.model <-
  expand.grid(
    sample_source = sample_sources[!sample_sources$reference,"sample_source"],
...
dens.model <- ddply(dens.model,
  ...
  don_chem_type <- as.character(df$don_chem_type[1])
	...
	#cat("Computing polynomial for (", sample_source, ",", acc_chem_type, ",", don_chem_type, ") ...\n", sep="")

  poly <- get_1d_polynomial(
    polynomials[as.character(polynomials$sample_source) == sample_source,],
  ...
  x <- seq(xmin, xmax, length.out=100)
	...
	z <- data.frame(
		...
		sample_source=factor(paste(sample_source, "Rosetta Model")))
...
dens <- rbind(dens_est, dens.model)
...
p <- ggplot(data=dens) + theme_bw() +
	...
	scale_x_continuous(expression(paste('Acceptor -- Proton Distance (', ring(A), ')')), breaks=c(1, 1.5, 2, 2.5)) +
	coord_trans(limx=c(1.4, 2.5), limy=c(0,8.5))
