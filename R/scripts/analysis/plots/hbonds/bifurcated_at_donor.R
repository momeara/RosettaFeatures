...
plot_parts <- list(
	theme_bw(),
	geom_point(aes(x=AHdist1, y=AHdist2), size=.7, colour="black"),
	stat_density2d(aes(x=AHdist1, y=AHdist2), size=.2, colour="blue"),
...
d_ply(f, .(sample_source), function(sub_f){
	...
	plot_id <- paste("hbond_bifurcated_at_donor_AHdist", ss_id, sep="_")
	...
	d_ply(sub_f, .(don_chem_type), function(sub_sub_f){
		...
		plot_id <- paste("hbond_bifurcated_at_",don_chem_type, "_AHdist_", ss_id, sep="")
	...
	plot_id <- paste("hbond_backbone_bifurcated_AHdist_", ss_id, sep="")
