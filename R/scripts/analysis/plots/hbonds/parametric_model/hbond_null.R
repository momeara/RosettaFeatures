...
dens_raw <- estimate_density_1d(
  ...
  variable = "length",
  histogram=TRUE)
dens_raw$normalization <- as.factor("Raw")
...
dens_normalized$normalization <- as.factor("Normalized")
...
ggplot(data=dens) + theme_bw() +
	geom_line(aes(x=x * 180/pi, y=y, color=normalization)) +
	ggtitle("Cosine of spherical Angle Null Model") +
	labs(x=expression(paste('Central Angle (', degree, ')')), y="Feature Density")
save_plots(self, plot_id, sample_sources, output_dir, output_formats)
